...
#ifndef MCAL_POLYHEDRAL_MESH_IO_H
#define MCAL_POLYHEDRAL_MESH_IO_H
...
// PolyhedralMesh的Input函数, 读取dat文件, 建立多面体网格.
// root_dir: 截断网格的根目录.
// 
template <typename PolyhedralMesh>
bool read_polyhedron_grid(std::string root_dir, PolyhedralMesh& pm)
{
	typedef typename PolyhedralMesh::Point       P;
	...
	if (!pm.load_from_truncated_grid(tg))
	{
...
// PolyhedralMesh的Output函数, 将多面体网格的数据按截断网格的文件格式写出.
// root_dir 截断网格的根目录
//
template <typename PolyhedralMesh>
bool write_polyhedron_grid(std::string out_path, PolyhedralMesh& pm, bool need_out)
{
	typedef typename PolyhedralMesh::Point       P;

	TruncatedGrid<P> tg;

	if (!pm.transform_to_truncated_grid(tg, need_out))
	{
