...
namespace MCAL {   // Mesh Cut Algorithm Libaray, 存放多面体网格切割算法的所有代码
	namespace IO {     // I/O
		...
		// 多面体网格没有独立的文件格式, 我们从截断网格的文件数据建立网格.
		// 二者在多面体网格的表示上有很多相似之处.
		// 截断网格用于在PolyhedronGrid的I/O过程存储相关的信息.



		/*
		* 截断网格也是一种多面体网格, 采用cell-face-vertex的层次描述地质体.
		* Code Source：从EsModelExport和EsTGUtility中提取需要的部分代码.
		* 作用：在PolyhedronGrid的I/O中记录几何数据, 以此建立PolyhedronGrid.
		*
		* 截断网格的概念以及数据成员的含义参见docs中的《截断网格数据格式》
		*
