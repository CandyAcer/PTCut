// $Intro: 本文件是切割算法的第二部分, 负责将表面网格和多面体网格的相交区域三角化.
...
#ifndef MCAL_ALGORITHM_INTERSECTION_VISITOR_H
#define MCAL_ALGORITHM_INTERSECTION_VISITOR_H
...
std::vector<int> sm_locate(3, 0);
std::vector<int> pg_locate(3, 0);
...
namespace MCAL   // Mesh Cut Algorithm Libaray, 存放多面体网格切割算法的实现
{
	...
	/*
	* IntersectionVisitor负责将表面网格和多面体网格的相交区域三角化, 并将三角化结果作用在网格中.
	* 三角化所需信息来自于IntersectionImpl, 因此作为它的数据成员(或者说Visitor), 在其算法过程中搜集相关信息.
	*
	* 算法过程如下:
	*
	*
	*/
	...
	class IntersectionVisitor
	{
		...
		typedef std::unordered_map<PGFace, NodeIdVec>        OnPGFace;
		typedef std::unordered_map<PGEdge, NodeIdVec>        OnPGEdge;
		typedef std::vector<PGVertex>                        NodeToPGVertex;
		typedef std::unordered_map<PGVertex, NodeId>         PGVertexToNode;
		typedef std::multimap<NodeId, PGHalfedge>            NodeToPGTarget;

		typedef IntersectionNodes<SurfaceMesh, PolyhedronGrid, VPMSM, VPMPG>    INodes;
	...
	private:
		...
		// 1) corresponding information. 交点到底落在什么位置, 注意sm和pg都各自有一份.
		...
		OnPGFace on_pg_face;  // 落在pg面内部的交点
		OnPGEdge on_pg_edge;  // 落在pg边上的交点
		NodeToPGVertex node_id_to_pg_vertex;   // node id与pg vertex的对应关系
		PGVertexToNode pg_vertex_to_node_id;   // pg vertex与node id的对应关系
		NodeToPGTarget node_to_pg_target_halfedge;  // 交点与指向它的半边
	...
	public:
		IntersectionVisitor(OutputBuilder& ob) :output_builder(ob) {}

		// 当surface mesh edge 与 polyhedron grid face相交时, 根据相交类型, 记录对应关系.
			...
			bool is_target_coplanar)
		{
			...
			switch (type)
			{
			case ON_FACE: // 当sm edge交在pg face的内部时, 要记录在on_pg_face中.
			{
				on_pg_face[pg.face(h_pg)].push_back(node_id);
				++pg_locate[0];
			}
			break;

			case ON_EDGE: // 当sm edge交在pg face的某条边上时, 要记录在on_pg_edge中.
			{
				on_pg_edge[pg.edge(h_pg)].push_back(node_id);
				++pg_locate[1];
			}
			break;

			case ON_VERTEX: // 当sm edge交在pg face的某个顶点上时, 要记录vertex与node id的对应关系.
			{
				++pg_locate[2];

				node_to_pg_target_halfedge.insert(std::make_pair(node_id, h_pg));
				if (node_id_to_pg_vertex.size() <= node_id)
					node_id_to_pg_vertex.resize(node_id + 1, pg.null_vertex());
				node_id_to_pg_vertex[node_id] = pg.target(h_pg);
				bool insert_ok =
					pg_vertex_to_node_id.insert(std::make_pair(pg.target(h_pg), node_id)).second;
				assert(insert_ok || pg_vertex_to_node_id[pg.target(h_pg)] == node_id);

				// 注意, 此时交点对应的vertex在多面体网格中还没有, 要在三角化的过程中逐步创建并由该函数记录.
				output_builder.set_pg_vertex_id(pg.target(h_pg), node_id);
			}
			break;
		...
		// 当polyhedron grid edge 与 surface mesh face相交时, 根据相交类型, 记录对应关系.
			...
			bool is_target_coplanar)
		{
			...
			else if (is_source_coplanar)
			{
				...
				PGHalfedge opp_h_pg = pg.polygon_mate(h_pg);
				node_to_pg_target_halfedge.insert(std::make_pair(node_id, opp_h_pg));
				if (node_id_to_pg_vertex.size() <= node_id)
					node_id_to_pg_vertex.resize(node_id + 1, pg.null_vertex());
				node_id_to_pg_vertex[node_id] = pg.target(opp_h_pg);
				bool insert_ok =
					pg_vertex_to_node_id.insert(std::make_pair(pg.target(opp_h_pg), node_id)).second;
				assert(insert_ok || pg_vertex_to_node_id[pg.target(opp_h_pg)] == node_id);

				output_builder.set_pg_vertex_id(pg.source(h_pg), node_id);
			}
			else
			{
				on_pg_edge[pg.edge(h_pg)].push_back(node_id);
		...
		void record_segment_cell(SortedPair<NodeId>& segment, PGCell cell)
		{
		...
		struct PGFaceBoundary
		{
			PGVertex                     vertices[3];
			PGHalfedge                   halfedges[3];
			std::map<PGHalfedge, int>    hedge_id_map;
			std::vector<NodeId>          ids_on_edge[3];
			std::map<SortedPair<PGVertex>, PGEdge>      endpoint_to_edge;

			PGFaceBoundary(PGHalfedge first, PolyhedronGrid& pg)
			{
				halfedges[0] = first;
				halfedges[1] = pg.next(first);
				halfedges[2] = pg.next(halfedges[1]);

				vertices[0] = pg.source(halfedges[0]);
				vertices[1] = pg.source(halfedges[1]);
				vertices[2] = pg.source(halfedges[2]);
				...
				for (int i = 0; i < 3; ++i)
				{
					SortedPair<PGVertex> vpair(vertices[i], vertices[(i + 1) % 3]);
					endpoint_to_edge.insert(std::make_pair(vpair, pg.edge(halfedges[i])));
				}
			}

			template <typename Iterator>
			void copy_ids_on_edge(PGHalfedge h, Iterator begin, Iterator end)
			{
				typename std::map<PGHalfedge, int>::iterator it
					= hedge_id_map.find(h);
				assert(it != hedge_id_map.end());
				std::copy(begin, end, std::back_inserter(ids_on_edge[it->second]));
			}

			void record_endpoint_to_edge(PGVertex v1, PGVertex v2, PGEdge e)
			{
				SortedPair<PGVertex> vpair(v1, v2);
				endpoint_to_edge.insert(std::make_pair(vpair, e));
			}

			void remove_endpoint_to_edge(PGVertex v1, PGVertex v2)
			{
				SortedPair<PGVertex> vpair(v1, v2);
		...
		typedef std::unordered_map<PGFace, PGFaceBoundary> PGFaceBoundaries;

		// 对h_split上的点进行排序, 按从远到近排序.
		// 即排序后ids_on_edge中索引越小的点离pg.target(h_split)越远.
		void sort_vertices_along_pg_halfedge(NodeIdVec& ids_on_edge,
			PGHalfedge h_split,
			PolyhedronGrid& pg,
			VPMPG& vpm_pg,
			INodes& nodes)
		{
			std::sort(ids_on_edge.begin(),
				ids_on_edge.end(),
				Less_along_a_pg_halfedge<PolyhedronGrid, VPMPG, INodes>
				(h_split, pg, vpm_pg, nodes));
		}

		// split every intersected edge in polyhedron grid.
		// 
		// Note: 与split_sm_halfedges()不同, split_pg_halfedges()并不打断边, 也不调整拓扑.
		// 本函数结束后多面体网格的拓扑没有任何改变, 我们只是收集新边与其端点等信息.
		//
		void split_pg_halfedges(OnPGEdge& on_pg_edge,
			PolyhedronGrid& pg,
			VPMPG& vpm_pg,
			INodes& nodes,
			PGFaceBoundaries& pg_face_boundaries)
		{
			for (typename OnPGEdge::iterator it = on_pg_edge.begin();
				it != on_pg_edge.end();
				++it)
			{
				PGEdge e_split = it->first;
				NodeIdVec& ids_on_edge = it->second;

				// 确保ids_on_edge中没有重复的node_id.
				assert(std::set<NodeId>(ids_on_edge.begin(), ids_on_edge.end()).size() == ids_on_edge.size());

				PGHalfedge h_split = pg.halfedge(e_split);
				sort_vertices_along_pg_halfedge(ids_on_edge, h_split, pg, vpm_pg, nodes);

				std::vector<std::pair<PGFace, bool>> split_faces;
				pg.collect_split_faces(h_split, split_faces);

				for (std::size_t i = 0; i < split_faces.size(); ++i)
				{
					PGFace f = split_faces[i].first;
					assert(pg.is_triangle(f));
					bool is_same_direction = split_faces[i].second;

					PGHalfedge h = pg.halfedge(f);
					while (pg.edge(h) != e_split)
						h = pg.next(h);

					if (is_same_direction)
						assert(pg.target(h) == pg.target(h_split) && pg.source(h) == pg.source(h_split));
					else
						assert(pg.target(h) == pg.source(h_split) && pg.source(h) == pg.target(h_split));

					typename PGFaceBoundaries::iterator it_fb = pg_face_boundaries.find(f);
					if (it_fb == pg_face_boundaries.end())
					{
						PGFaceBoundary face_bound(h, pg);
						it_fb = pg_face_boundaries.insert(std::make_pair(f, face_bound)).first;
					}
				...
				PGVertex original_srcv = pg.source(h_split);
				PGVertex original_tgtv = pg.target(h_split);

				PGVertex pre_vertex = pg.source(h_split);
				for (NodeId node_id : ids_on_edge)
				{
					PGEdge enew = pg.add_edge();
					PGVertex vnew = pg.add_vertex();
					nodes.call_put(vpm_pg, vnew, node_id, pg);
					output_builder.set_pg_vertex_id(vnew, node_id);
					node_id_to_pg_vertex[node_id] = vnew;

					for (std::size_t i = 0; i < split_faces.size(); ++i)
					{
						typename PGFaceBoundaries::iterator it_fb =
							pg_face_boundaries.find(split_faces[i].first);
						assert(it_fb != pg_face_boundaries.end());
				...
				// 确保没有修改原来的拓扑.
				assert(pg.source(h_split) == original_srcv);
				assert(pg.target(h_split) == original_tgtv);
		...
		void update_pg_face_indices(std::array<PGVertex, 3>& f_vertices,
			std::array<NodeId, 3>& f_indices)
		{
			for (int i = 0; i < 3; ++i)
			{
				typename PGVertexToNode::iterator it =
					pg_vertex_to_node_id.find(f_vertices[i]);
				if (it != pg_vertex_to_node_id.end())
					f_indices[i] = it->second;
			}
		}

		void collect_triangulation_info(PGFace current_face,
			std::vector<NodeId>& ids_in_face,
			CDT& cdt,
			PolyhedronGrid& pg,
			VPMPG& vpm_pg,
			INodes& nodes,
			std::unordered_map<PGFace, std::vector<PGVertex>>& f_to_vseq,
			std::map<SortedPair<PGVertex>, PGEdge>& endpoint_to_edge)
		{
			...
			for (NodeId node_id : ids_in_face)
			{
				PGVertex v = pg.add_vertex();
				nodes.call_put(vpm_pg, v, node_id, pg);
				output_builder.set_pg_vertex_id(v, node_id);

				assert(node_id < node_id_to_pg_vertex.size());
				node_id_to_pg_vertex[node_id] = v;
			}
				...
				++it)
			{
				...
				if (!cdt.is_infinite(it->first->vertex(it->second)) &&
					!cdt.is_infinite(cdt.mirror_vertex(it->first, it->second)))
				{
					PGEdge e = pg.add_edge();

					NodeId i0 = cdt_v0->info(), i1 = cdt_v1->info();
					assert(node_id_to_pg_vertex[i0] != pg.null_vertex());
					assert(node_id_to_pg_vertex[i1] != pg.null_vertex());

					PGVertex v0 = node_id_to_pg_vertex[i0];
					PGVertex v1 = node_id_to_pg_vertex[i1];

					endpoint_to_edge.insert(std::make_pair(SortedPair<PGVertex>(v0, v1), e));
				}
			}
				...
				it_end = cdt.finite_faces_end();;)
			{
				...
				assert(node_id_to_pg_vertex[i0] != pg.null_vertex());
				assert(node_id_to_pg_vertex[i1] != pg.null_vertex());
				assert(node_id_to_pg_vertex[i2] != pg.null_vertex());

				PGVertex v0 = node_id_to_pg_vertex[i0];
				PGVertex v1 = node_id_to_pg_vertex[i1];
				PGVertex v2 = node_id_to_pg_vertex[i2];

				std::vector<PGVertex> vseq{ v0,v1,v2 };
				f_to_vseq.insert(std::make_pair(current_face, vseq));

				if (++it != it_end)
				{
					current_face = pg.add_face();
				}
				else
					break;
			}
		...
		void triangulate_pg_intersected_faces(OnPGFace& on_pg_face,
			PolyhedronGrid& pg,
			VPMPG& vpm_pg,
			INodes& nodes,
			PGFaceBoundaries& pg_face_boundaries)
		{
			...
			std::map<NodeIdPair, std::pair<PGVertex, PGEdge>> build_info;

			for (typename OnPGFace::iterator it = on_pg_face.begin();
				it != on_pg_face.end();
				++it)
			{
				PGFace f = it->first;
				...
				std::map<NodeId, CDT_Vertex_handle> id_to_CDT_vh;
				std::map<SortedPair<PGVertex>, PGEdge> endpoint_to_edge;
				std::array<PGVertex, 3> f_vertices;
				std::array<NodeId, 3> f_indices = { nb_nodes, nb_nodes + 1, nb_nodes + 2 };
				...
				if (it_fb != pg_face_boundaries.end())
				{
					...
					update_pg_face_indices(f_vertices, f_indices);
					endpoint_to_edge = it_fb->second.endpoint_to_edge;
				}
				else
				{
					PGHalfedge h0 = pg.halfedge(f), h1 = pg.next(h0), h2 = pg.next(h1);
					f_vertices[0] = pg.target(h0);
					f_vertices[1] = pg.target(h1);
					f_vertices[2] = pg.target(h2);

					update_pg_face_indices(f_vertices, f_indices);

					endpoint_to_edge.insert(std::make_pair(SortedPair<PGVertex>(f_vertices[2], f_vertices[0]), pg.edge(h0)));
					endpoint_to_edge.insert(std::make_pair(SortedPair<PGVertex>(f_vertices[0], f_vertices[1]), pg.edge(h1)));
					endpoint_to_edge.insert(std::make_pair(SortedPair<PGVertex>(f_vertices[1], f_vertices[2]), pg.edge(h2)));
				}

				typename EK::Point_3 p = nodes.to_exact(vpm_pg[f_vertices[0]]);
				typename EK::Point_3 q = nodes.to_exact(vpm_pg[f_vertices[1]]);
				typename EK::Point_3 r = nodes.to_exact(vpm_pg[f_vertices[2]]);
				...
				node_id_to_pg_vertex[nb_nodes] = f_vertices[0];
				node_id_to_pg_vertex[nb_nodes + 1] = f_vertices[1];
				node_id_to_pg_vertex[nb_nodes + 2] = f_vertices[2];
				...
				if (it_fb != pg_face_boundaries.end())
				{
					...
					PGFaceBoundary& f_bound = it_fb->second;
					for (int i = 0; i < 3; ++i)
					{
						NodeIdVec& ids_on_edge = f_bound.ids_on_edge[i];
						CDT_Vertex_handle prev_CDT_vh = triangle_vertices[i];
						...
						//else
						//{
						//	PGHalfedge h = f_bound.halfedges[i];
						//	assert(pg.target(h) == f_bound.vertices[(i+1)%3]);
						//	assert(pg.source(h) == f_bound.vertices[i]);
						//}
					}
				}
				...
				std::unordered_map<PGFace, std::vector<PGVertex>> f_to_vseq;
				collect_triangulation_info(f, ids_in_face, cdt, pg, vpm_pg, nodes, f_to_vseq, endpoint_to_edge);

				pg.import_triangulation_info(f, f_to_vseq, endpoint_to_edge);
				...
				for (const NodeIdPair& id_pair : constrained_edges)
				{
					...
					PGVertex v0 = node_id_to_pg_vertex[i0];
					PGVertex v1 = node_id_to_pg_vertex[i1];
					SortedPair<PGVertex> vpair(v0, v1);

					typename std::map<SortedPair<PGVertex>, PGEdge>::iterator
			...
			for (typename std::map<NodeIdPair, std::pair<PGVertex, PGEdge>>::iterator
				it = build_info.begin(); it != build_info.end(); ++it)
			{
				NodeIdPair id_pair = it->first;
				PGVertex srcv = it->second.first;
				PGEdge target_edge = it->second.second;
				output_builder.set_pg_edge_per_polyline(id_pair, srcv, target_edge, pg);
			}
		}
			...
			VPMPG& vpm_pg)
		{
			...
			node_id_to_pg_vertex.resize(nb_nodes + 3, pg.null_vertex());

			SMFaceBoundaries sm_face_boundaries;
			PGFaceBoundaries pg_face_boundaries;

			std::cout << "交点的总数量：" << nb_nodes << std::endl;
			std::cout << "落在sm_face内部的交点数量：" << sm_locate[0] << std::endl
				<< "落在sm_edge内部的交点数量：" << sm_locate[1] << std::endl
				<< "落在sm_vertex上的交点数量：" << sm_locate[2] << std::endl
				<< "落在pg_face内部的交点数量：" << pg_locate[0] << std::endl
				<< "落在pg_edge内部的交点数量：" << pg_locate[1] << std::endl
				<< "落在pg_vertex上的交点数量：" << pg_locate[2] << "\n\n";
			...
			split_pg_halfedges(on_pg_edge, pg, vpm_pg, nodes, pg_face_boundaries);

			std::cout << "打断边的过程----------------------------" << std::endl;
			...
			cur_exact_cnt = iExactComputeCount;
			cur_interval_cnt = iIntervalComputeCount;

			triangulate_sm_intersected_faces(on_sm_face, sm, vpm_sm, nodes, sm_face_boundaries);
			triangulate_pg_intersected_faces(on_pg_face, pg, vpm_pg, nodes, pg_face_boundaries);

			std::cout << "三角化过程----------------------------" << std::endl;
			std::cout << "精确计算的次数：" << iExactComputeCount - cur_exact_cnt << std::endl;
			std::cout << "区间代数的次数：" << iIntervalComputeCount - cur_interval_cnt << std::endl;
			std::cout << "----------------------------------------\n\n";

			nodes.finalize(node_id_to_sm_vertex, node_id_to_pg_vertex);

			node_id_to_sm_vertex.resize(nb_nodes);
			node_id_to_pg_vertex.resize(nb_nodes);
			output_builder(nodes, node_id_to_sm_vertex, node_id_to_pg_vertex);
		}
